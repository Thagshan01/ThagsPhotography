    ...
    <header className="border-b border-neutral-900">
      <div className="container flex items-center justify-between py-5 lg:pb-0">
        ...
        <nav className={`navbar ${isOpen ? "active" : ""}`}>
          ...
          <ul className="grid">
            {navItems.map((item) => (
              <li key={item.id} className="border-b border-neutral-900 text-center">
                <Link href={item.path} className={`block py-5 hover:bg-neutral-900 ${pathname === item.path ? "bg-neutral-900" : ""}`}
                  onClick={handleClick}>
                  {item.label}
                </Link>
              </li>
          ...
          {/* Contact */}
          <Link href="/contact" className="primary-btn block text-center mt-10">
            contact me
          </Link>
        </nav>
